...
protected:
  ...
  virtual double computeExtractionCost(Superword *superword,
...
/// The unit cost model assumes cost of 1 for every MLIR operation. This means,
/// that if a pattern lowers some superword to a group of 5 statements in
/// whatever dialect, the pattern's cost will be 5. It does not take into
/// account how many LLVM operations or even machine code instructions these 5
/// will then be lowered to.
class UnitCostModel : public CostModel {
  using CostModel::CostModel;
  double computeScalarCost(Value value) const override;
  double computeExtractionCost(Superword *superword,
  ...
  void visit(VectorizeConstant const *pattern,
             Superword const *superword) override;
  void visit(VectorizeGaussian const *pattern,
             Superword const *superword) override;
  void visit(VectorizeLogAdd const *pattern,
             Superword const *superword) override;
  void visit(VectorizeLogGaussian const *pattern,
             Superword const *superword) override;
};
} // namespace slp
} // namespace low

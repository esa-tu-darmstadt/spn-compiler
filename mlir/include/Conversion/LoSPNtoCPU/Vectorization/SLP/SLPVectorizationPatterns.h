...
template <typename SourceOp, typename... CompatibleOps>
class LogSpaceVectorizationPattern
    : public OpSpecificVectorizationPattern<SourceOp, CompatibleOps...> {
  using OpSpecificVectorizationPattern<
      SourceOp, CompatibleOps...>::OpSpecificVectorizationPattern;
...
private:
  ...
  DenseMap<Superword *,
           std::tuple<Superword *, Superword *, SmallVector<int64_t, 4>>>
      shuffleMatches;
};
...
struct VectorizeConstant
    : public OpSpecificVectorizationPattern<arith::ConstantOp, SPNConstant> {
  using OpSpecificVectorizationPattern<
      arith::ConstantOp, SPNConstant>::OpSpecificVectorizationPattern;
  Value rewrite(Superword *superword, RewriterBase &rewriter) override;
  void accept(PatternVisitor &visitor,
              Superword const *superword) const override;
};
...
struct CreateConsecutiveLoad
    : public OpSpecificVectorizationPattern<SPNBatchRead> {
  using OpSpecificVectorizationPattern<
      SPNBatchRead>::OpSpecificVectorizationPattern;
  LogicalResult match(Superword *superword) override;
  Value rewrite(Superword *superword, RewriterBase &rewriter) override;
  void accept(PatternVisitor &visitor,
              Superword const *superword) const override;
...
struct VectorizeAdd : public NormalSpaceVectorizationPattern<SPNAdd> {
  using NormalSpaceVectorizationPattern<
      SPNAdd>::NormalSpaceVectorizationPattern;
  Value rewrite(Superword *superword, RewriterBase &rewriter) override;
  void accept(PatternVisitor &visitor,
              Superword const *superword) const override;
};
...
struct VectorizeMul : public NormalSpaceVectorizationPattern<SPNMul> {
  using NormalSpaceVectorizationPattern<
      SPNMul>::NormalSpaceVectorizationPattern;
  Value rewrite(Superword *superword, RewriterBase &rewriter) override;
  void accept(PatternVisitor &visitor,
              Superword const *superword) const override;
};

/// Vectorization pattern for creating vectorized normal distribution
/// evaluations in normal space.
struct VectorizeGaussian
    : public NormalSpaceVectorizationPattern<SPNGaussianLeaf> {
  using NormalSpaceVectorizationPattern<
      SPNGaussianLeaf>::NormalSpaceVectorizationPattern;
  Value rewrite(Superword *superword, RewriterBase &rewriter) override;
  void accept(PatternVisitor &visitor,
              Superword const *superword) const override;
};
...
/// Vectorization pattern for creating vectorized normal distribution
/// evaluations in log space.
struct VectorizeLogGaussian
    : public LogSpaceVectorizationPattern<SPNGaussianLeaf> {
  using LogSpaceVectorizationPattern<
      SPNGaussianLeaf>::LogSpaceVectorizationPattern;
  Value rewrite(Superword *superword, RewriterBase &rewriter) override;
  void accept(PatternVisitor &visitor,
              Superword const *superword) const override;
};
...
static inline SmallVector<std::unique_ptr<SLPVectorizationPattern>>
allSLPVectorizationPatterns(ConversionManager &conversionManager) {
  SmallVector<std::unique_ptr<SLPVectorizationPattern>> patterns;
  // === Op-agnostic patterns === //
  patterns.emplace_back(
      std::make_unique<BroadcastSuperword>(conversionManager));
  patterns.emplace_back(
      std::make_unique<BroadcastInsertSuperword>(conversionManager));
  patterns.emplace_back(
      std::make_unique<ShuffleTwoSuperwords>(conversionManager));
  // === Op-specific patterns === //
  ...
  patterns.emplace_back(
      std::make_unique<VectorizeLogGaussian>(conversionManager));
  // ====================================== //
  return patterns;
}

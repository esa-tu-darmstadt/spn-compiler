  ...
  namespace spn {
    namespace low {
      namespace slp {
        ...
        /// A superword models an SLP vector, which basically consists of a fixed amount of operations (depending on the
        /// hardware's supported vector width) that can be computed isomorphically. Every superword tracks its operand
        /// superwords, i.e. those that it would use for computation of its own elements. The term 'superword' was
        /// chosen because a 'vector' is quite overloaded in C++.
        class Superword {
        ...
        public:
          ...
          /// Replace the superword's value in the requested lane with the provided value.
          ...
          /// Check if the superword is a leaf superword, i.e. if it does not have any superword operands.
          bool isLeaf() const;
          ...
          /// Retrieve the operand superword at the index's position (the index-th operand).
          Superword* getOperand(size_t index) const;
          /// Retrieve all operands of the superword.
          SmallVector<Superword*, 2> getOperands() const;

          /// Check if the superword has had its semantics altered in the requested lane. This can happen if the
          /// computation chain of its scalar element in that lane does not correspond to the computation chain of the
          /// elements in the superword's operand chain anymore (e.g. through reordering of elements in the superword's
          /// operands).
          ...
          /// Return the location of the superword in the program. This always corresponds to the location of the first
          /// element.
        ...
        /// The dependency graph models flow dependencies of an SLP graph. Its nodes are superwords. Edges exist between
        /// superwords if at least one element of the source superword 'flows into' at least one element in the
        /// destination superword (i.e. it appears in the computation chain of the destination element).
        struct DependencyGraph {
          /// Return the number of nodes in the dependency graph.
          size_t numNodes() const;
          /// Return the number of edges in the dependency graph.
          size_t numEdges() const;
          /// Return the dependency graph's superwords in post order. Here, a superword s1 always appears before an
          /// other superword s2 if there is no dependency edge from s2 to s1. If there is no edge between them, the
          /// superwords are sorted by how often they are used as destination of an edge.
        ...
        /// This class models the nodes in an SLP graph. Each SLP node consists of one or more superwords and can have
        /// zero or more operand nodes. It can be understood as a kind of superstructure around the graph modeled by
        /// the superwords themselves.
        class SLPNode {
        ...
        public:
          ...
          /// Replace the element of the superword at the requested index and lane with the provided value.
          void setValue(size_t lane, size_t index, Value newValue);

          /// Check if the node contains the requested value.
          bool contains(Value value) const;

          /// Check if the provided superword is the root superword of the node (the very first one).
        ...
        /// The SLP graph models a graph whose nodes are SLP nodes and whose edges are computation flows between the
        /// nodes' superwords.
        class SLPGraph {
        ...
        namespace graph {
          ...
          /// Walks through a graph rooted at node 'root' in post order and applies function 'f' to every visited node.
          template<typename Node, typename Function>
          void walk(Node* root, Function f) {
            // Use a stack-based approach instead of recursion. Whenever a (node, true) pair is popped from the stack,
            // the function is called on the node since then its operands have all been handled already.
            // TL;DR: false = visit node operands first, true = finished

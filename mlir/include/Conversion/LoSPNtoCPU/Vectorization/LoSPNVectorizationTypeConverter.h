...
public:
  explicit LoSPNVectorizationTypeConverter(unsigned vector_width) {
    ...
    addTargetMaterialization([](OpBuilder &builder, VectorType type,
      ...
      if (auto toScalar = dyn_cast<low::SPNConvertToScalar>(
              inputs.front().getDefiningOp())) {
        // Handle the special case that the values was previously converted from
        // a vector to a scalar.
        assert(toScalar.getVector().getType() == type);
        return toScalar.getVector();
      }
      return builder.create<low::SPNConvertToVector>(loc, type, inputs.front())
          .getResult();
    });
  }
};

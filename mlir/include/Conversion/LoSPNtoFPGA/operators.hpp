...
inline const std::array<OperatorType, 9> OPERATOR_TYPES = {
    ...
    TYPE_HISTOGRAM, TYPE_CONSTANT, TYPE_YIELD, TYPE_BODY};
...
enum OperatorPortDirection { INPUT, OUTPUT };
...
class OperatorTypeMapping {
  ...
  static OperatorPortInfo inPort(const std::string &name,
                                 OperatorPortType type) {
    return OperatorPortInfo{.name = name, .direction = INPUT, .type = type};
  }

  static OperatorPortInfo outPort(const std::string &name,
                                  OperatorPortType type) {
...
public:
  OperatorTypeMapping() {
    ...
    delays = {{TYPE_ADD, 6},      {TYPE_MUL, 5},         {TYPE_LOG, 0},
              {TYPE_CONVERT, 2},  {TYPE_CATEGORICAL, 1}, {TYPE_CONSTANT, 0},
              {TYPE_HISTOGRAM, 1}};
  }
  ...
  uint64_t getDelay(std::string &baseName) const {
    return getDelay(baseNameToType.at(baseName));
  }

  virtual std::vector<OperatorPortInfo>
  getOperatorPorts(OperatorType type) const {
    switch (type) {
    case TYPE_ADD:
    case TYPE_MUL:
      return std::vector<OperatorPortInfo>{
          inPort("clock", PORT_SIGNAL), inPort("reset", PORT_SIGNAL),
          inPort("io_a", PORT_PROBABILITY), inPort("io_b", PORT_PROBABILITY),
          outPort("io_r", PORT_PROBABILITY)};
    case TYPE_CATEGORICAL:
    case TYPE_HISTOGRAM:
      return std::vector<OperatorPortInfo>{
          inPort("clk", PORT_SIGNAL), inPort("rst", PORT_SIGNAL),
    ...
    case TYPE_LOG:
      return std::vector<OperatorPortInfo>{
          inPort("clk", PORT_SIGNAL), inPort("rst", PORT_SIGNAL),
          inPort("in_a", PORT_PROBABILITY), outPort("out_b", PORT_PROBABILITY)};
    case TYPE_CONVERT:
      return std::vector<OperatorPortInfo>{
          inPort("clock", PORT_SIGNAL), inPort("reset", PORT_SIGNAL),
          inPort("io_op", PORT_PROBABILITY), outPort("io_r", PORT_FLOAT64)};
    case TYPE_BODY:
      return std::vector<OperatorPortInfo>{inPort("clk", PORT_SIGNAL),
  ...
  static OperatorType getOperatorType(Operation *op) {
    OperatorType opType =
        llvm::TypeSwitch<Operation *, OperatorType>(op)
            .Case<SPNAdd>([&](SPNAdd op) { return TYPE_ADD; })
            .Case<SPNMul>([&](SPNMul op) { return TYPE_MUL; })
            .Case<SPNCategoricalLeaf>(
                [&](SPNCategoricalLeaf op) { return TYPE_CATEGORICAL; })
            .Case<SPNConstant>([&](SPNConstant op) { return TYPE_CONSTANT; })
            .Case<SPNLog>([&](SPNLog op) { return TYPE_LOG; })
            .Case<SPNHistogramLeaf>(
                [&](SPNHistogramLeaf op) { return TYPE_HISTOGRAM; })

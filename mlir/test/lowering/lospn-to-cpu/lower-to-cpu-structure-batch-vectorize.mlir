// RUN: %optcall -pass-pipeline="builtin.module(convert-lospn-structure-to-cpu{cpu-vectorize=true})" %s | FileCheck %s
  ...
  ^bb0(%arg0: memref<?x6xf64>, %arg1: memref<1x?xf64>):  // no predecessors
    ...
    ^bb0(%arg2: index, %arg3: memref<?x6xf64>, %arg4: memref<1x?xf64>):  // no predecessors
      ...
      ^bb0(%arg5: f64, %arg6: f64, %arg7: f64, %arg8: f64, %arg9: f64, %arg10: f64):  // no predecessors
        ...
        %13 = "lo_spn.histogram"(%arg7) {bucketCount = 2 : ui32, buckets = [#lo_spn.bucket<0, 1, 2.500000e-01>, #lo_spn.bucket<1, 2, 7.500000e-01>], supportMarginal = false} : (f64) -> f64
        %14 = "lo_spn.histogram"(%arg8) {bucketCount = 2 : ui32, buckets = [#lo_spn.bucket<0, 1, 4.500000e-01>, #lo_spn.bucket<1, 2, 5.500000e-01>], supportMarginal = false} : (f64) -> f64
    ...
    "lo_spn.return"() : () -> ()
  }) {sym_name = "spn_vector", function_type = (memref<?x6xf64>, memref<1x?xf64>) -> ()} : () -> ()
}
...
// CHECK:             %[[VAL_18:.*]] = "lo_spn.histogram"(%[[VAL_12]]) {bucketCount = 2 : ui32, buckets = [#lo_spn.bucket<0, 1, 2.500000e-01>, #lo_spn.bucket<1, 2, 7.500000e-01>], supportMarginal = false, vector_width = 4 : i32} : (f64) -> f64
// CHECK:             %[[VAL_19:.*]] = "lo_spn.histogram"(%[[VAL_13]]) {bucketCount = 2 : ui32, buckets = [#lo_spn.bucket<0, 1, 4.500000e-01>, #lo_spn.bucket<1, 2, 5.500000e-01>], supportMarginal = false, vector_width = 4 : i32} : (f64) -> f64
...
// CHECK:             %[[VAL_42:.*]] = "lo_spn.histogram"(%[[VAL_36]]) {bucketCount = 2 : ui32, buckets = [#lo_spn.bucket<0, 1, 2.500000e-01>, #lo_spn.bucket<1, 2, 7.500000e-01>], supportMarginal = false} : (f64) -> f64
// CHECK:             %[[VAL_43:.*]] = "lo_spn.histogram"(%[[VAL_37]]) {bucketCount = 2 : ui32, buckets = [#lo_spn.bucket<0, 1, 4.500000e-01>, #lo_spn.bucket<1, 2, 5.500000e-01>], supportMarginal = false} : (f64) -> f64

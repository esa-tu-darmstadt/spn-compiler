...
// CHECK:           %[[VAL_3:.*]] = "lo_spn.batch_read"(%[[VAL_0]], %[[VAL_2]]) {sampleIndex = 0 : ui32} : (memref<?x2xi32>, index) -> i32
// CHECK:           %[[VAL_4:.*]] = "lo_spn.batch_read"(%[[VAL_0]], %[[VAL_2]]) {sampleIndex = 1 : ui32} : (memref<?x2xi32>, index) -> i32
// CHECK:           %[[VAL_5:.*]] = "lo_spn.histogram"(%[[VAL_3]]) {bucketCount = 2 : ui32, buckets = [{lb = 0 : i32, ub = 1 : i32, val = 2.500000e-01 : f64}, {lb = 1 : i32, ub = 2 : i32, val = 7.500000e-01 : f64}], supportMarginal = false} : (i32) -> f64
// CHECK:           %[[VAL_6:.*]] = "lo_spn.histogram"(%[[VAL_4]]) {bucketCount = 2 : ui32, buckets = [{lb = 0 : i32, ub = 1 : i32, val = 4.500000e-01 : f64}, {lb = 1 : i32, ub = 2 : i32, val = 5.500000e-01 : f64}], supportMarginal = false} : (i32) -> f64
// CHECK:           %[[VAL_7:.*]] = "lo_spn.mul"(%[[VAL_5]], %[[VAL_6]]) : (f64, f64) -> f64
// CHECK:           %[[VAL_8:.*]] = "lo_spn.log"(%[[VAL_7]]) : (f64) -> f64
// CHECK:           "lo_spn.batch_write"(%[[VAL_8]], %[[VAL_1]], %[[VAL_2]]) : (f64, memref<?xf64>, index) -> ()
// CHECK:           "lo_spn.return"() : () -> ()
// CHECK:         }

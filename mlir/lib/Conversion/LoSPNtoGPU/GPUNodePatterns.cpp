...
mlir::LogicalResult mlir::spn::BatchReadGPULowering::matchAndRewrite(
  ...
  auto constStaticIndex = rewriter.create<ConstantOp>(
...
mlir::LogicalResult mlir::spn::BatchWriteGPULowering::matchAndRewrite(
  ...
  assert(operands.size() == op.getResultValues().size() + 2 &&
         "Expecting correct number of operands for BatchWrite");
  ...
  for (unsigned i = 0; i < op.getResultValues().size(); ++i) {
    ...
    auto constStaticIndex =
        rewriter.create<ConstantOp>(op.getLoc(), rewriter.getIndexAttr(i));
    if (transposed) {
      indices.push_back(constStaticIndex);
    ...
    rewriter.create<memref::StoreOp>(op.getLoc(), operands[i + 2], memRef,
                                     indices);
  }
  rewriter.eraseOp(op);
  return success();
...
mlir::LogicalResult mlir::spn::CopyGPULowering::matchAndRewrite(
    mlir::spn::low::SPNCopy op, mlir::spn::low::SPNCopy::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  ...
  if (srcType.getRank() != tgtType.getRank() || srcType.getRank() != 1) {
    return rewriter.notifyMatchFailure(op,
                                       "Expecting one dimensional memories");
  }
  ...
  auto load = rewriter.create<memref::LoadOp>(op.getLoc(), op.source(),
                                              outer.getInductionVar());
  (void)rewriter.create<memref::StoreOp>(op.getLoc(), load, op.getTarget(),
                                         outer.getInductionVar());
  rewriter.eraseOp(op);
  return success();
}
...
mlir::LogicalResult mlir::spn::ConstantGPULowering::matchAndRewrite(
...
mlir::LogicalResult mlir::spn::ReturnGPULowering::matchAndRewrite(
    mlir::spn::low::SPNReturn op, mlir::spn::low::SPNReturn::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  if (!operands.empty()) {
    // At this point, all Tensor semantic should have been removed by the
    // bufferization. Hence, the SPNReturn, which can only return Tensors,
    // should not have any return values anymore and should merely be used as a
    // terminator for Kernels and Tasks.
    return rewriter.notifyMatchFailure(
        op, "SPNReturn can only return Tensors, which should have been removed "
            "by bufferization");
  }
...
mlir::LogicalResult mlir::spn::LogGPULowering::matchAndRewrite(
    mlir::spn::low::SPNLog op, mlir::spn::low::SPNLog::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  if (op.checkVectorized()) {
    return rewriter.notifyMatchFailure(op,
                                       "Pattern does not vectorize, no match");
...
mlir::LogicalResult mlir::spn::MulGPULowering::matchAndRewrite(
  ...
  if (!operands[0].getType().isa<FloatType>()) {
    return rewriter.notifyMatchFailure(
        op, "Currently only matches floating-point multiplications");
  }
  rewriter.replaceOpWithNewOp<MulFOp>(op, operands[0], operands[1]);
  return success();
}

mlir::LogicalResult mlir::spn::MulLogGPULowering::matchAndRewrite(
    mlir::spn::low::SPNMul op, mlir::spn::low::SPNMul::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  if (op.checkVectorized()) {
    return rewriter.notifyMatchFailure(op,
...
mlir::LogicalResult mlir::spn::AddGPULowering::matchAndRewrite(
  ...
  if (!operands[0].getType().isa<FloatType>()) {
    return rewriter.notifyMatchFailure(
        op, "Currently only matches floating-point additions");
  }
  rewriter.replaceOpWithNewOp<AddFOp>(op, operands[0], operands[1]);
  return success();
}

mlir::LogicalResult mlir::spn::AddLogGPULowering::matchAndRewrite(
    mlir::spn::low::SPNAdd op, mlir::spn::low::SPNAdd::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  if (op.checkVectorized()) {
    return rewriter.notifyMatchFailure(op,
  ...
  // b == log(y) and a > b.
  auto compare = rewriter.create<CmpFOp>(op.getLoc(), CmpFPredicate::OGT,
                                         operands[0], operands[1]);
  auto a = rewriter.create<SelectOp>(op->getLoc(), compare, operands[0],
                                     operands[1]);
  auto b = rewriter.create<SelectOp>(op->getLoc(), compare, operands[1],
                                     operands[0]);
  ...
  auto constantOne = rewriter.create<ConstantOp>(
      op.getLoc(), rewriter.getFloatAttr(operands[0].getType(), 1.0));
...
mlir::LogicalResult mlir::spn::GaussianGPULowering::matchAndRewrite(
  ...
  if (!operands.front().getType().isIntOrFloat()) {
    return rewriter.notifyMatchFailure(
        op, "Pattern only takes int or float as input");
  }
  if (!op.getResult().getType().isa<FloatType>()) {
    return rewriter.notifyMatchFailure(
        op, "Cannot match Gaussian computing non-float result");
  }
  ...
  if (indexType.isIntOrIndex()) {
    ...
    // Widden or narrow the index floating-point type to the result
    // floating-point type.
    ...
    // The input is neither float nor integer/index, fail this pattern because
    // no conversion is possible.
    return rewriter.notifyMatchFailure(
        op, "Match failed because input is neither float nor integer/index");
  }
  ...
  auto coefficientConst = rewriter.create<mlir::ConstantOp>(
      op.getLoc(), rewriter.getF64FloatAttr(coefficient));
  // -1/(2*variance)
  double denominator = -1.0 / (2.0 * variance);
  auto denominatorConst = rewriter.create<mlir::ConstantOp>(
      op.getLoc(), rewriter.getF64FloatAttr(denominator));
  // x - mean
  auto meanConst =
      rewriter.create<mlir::ConstantOp>(op.getLoc(), op.getMeanAttr());
  auto subtraction =
      rewriter.create<mlir::SubFOp>(op.getLoc(), index, meanConst);
  // (x-mean)^2
  auto numerator =
      rewriter.create<mlir::MulFOp>(op.getLoc(), subtraction, subtraction);
  // -(x-mean)^2 / 2*variance
  auto fraction =
      rewriter.create<mlir::MulFOp>(op.getLoc(), numerator, denominatorConst);
  ...
  Value gaussian =
      rewriter.create<mlir::MulFOp>(op->getLoc(), coefficientConst, exp);
  if (op.getSupportMarginal()) {
    auto isNan = rewriter.create<mlir::CmpFOp>(op->getLoc(), CmpFPredicate::UNO,
                                               index, index);
    auto constOne = rewriter.create<mlir::ConstantOp>(
        op.getLoc(), rewriter.getFloatAttr(resultType, 1.0));
    gaussian =
        rewriter.create<mlir::SelectOp>(op.getLoc(), isNan, constOne, gaussian);
  }
  rewriter.replaceOp(op, gaussian);
  return success();
}

mlir::LogicalResult mlir::spn::GaussianLogGPULowering::matchAndRewrite(
    mlir::spn::low::SPNGaussianLeaf op,
    mlir::spn::low::SPNGaussianLeaf::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  if (op.checkVectorized()) {
  ...
  // x - mean
  auto meanConst = rewriter.create<mlir::ConstantOp>(
      op.getLoc(),
      rewriter.getFloatAttr(resultType, op.getMeanAttr().getValueAsDouble()));
  auto subtraction =
      rewriter.create<mlir::SubFOp>(op.getLoc(), index, meanConst);
  // (x-mean)^2
  auto numerator =
  ...
  Value gaussian =
      rewriter.create<mlir::AddFOp>(op->getLoc(), coefficientConst, fraction);
  if (op.getSupportMarginal()) {
    auto isNan = rewriter.create<mlir::CmpFOp>(op->getLoc(), CmpFPredicate::UNO,
                                               index, index);
    auto constOne = rewriter.create<mlir::ConstantOp>(
        op.getLoc(), rewriter.getFloatAttr(resultType, 0.0));
    gaussian =
        rewriter.create<mlir::SelectOp>(op.getLoc(), isNan, constOne, gaussian);
  }
  rewriter.replaceOp(op, gaussian);
...
mlir::LogicalResult mlir::spn::CategoricalGPULowering::matchAndRewrite(
  ...
  if (!index.getType().isIntOrIndex()) {
    // If the input type is not an integer, but also not a float, we cannot
    // convert it and this pattern fails.
    if (!index.getType().isIntOrFloat()) {
      return rewriter.notifyMatchFailure(
          op, "Cannot convert input of Categorical to integer");
    }
    index = rewriter.template create<mlir::arith::FPToUIOp>(
        op.getLoc(), index, rewriter.getI64Type());
  }
  double defaultValue = (computesLog) ? static_cast<double>(-INFINITY) : 0;
  // TODO Replace 'getFloatAttr' with a more generic solution, if we want to
  // support integer computation.
  Value falseVal = rewriter.create<ConstantOp>(
      op.getLoc(), rewriter.getFloatAttr(resultType, defaultValue));
  auto probabilities = op.probabilitiesAttr().getValue();
  for (unsigned i = 0; i < op.getProbabilities().size(); ++i) {
    auto classVal =
        rewriter.create<ConstantOp>(op.getLoc(), rewriter.getI64IntegerAttr(i));
    auto cmp = rewriter.create<CmpIOp>(op.getLoc(), CmpIPredicate::eq, index,
                                       classVal);
    auto probability =
        probabilities[i].dyn_cast<FloatAttr>().getValueAsDouble();
    if (computesLog) {
      probability = log(probability);
    }
    auto probVal = rewriter.create<ConstantOp>(
        op.getLoc(), rewriter.getFloatAttr(resultType, probability));
  ...
  if (op.getSupportMarginal()) {
    ...
    auto isNan = rewriter.create<mlir::CmpFOp>(
        op->getLoc(), mlir::CmpFPredicate::UNO, indexOperand, indexOperand);
    auto marginalValue = (computesLog) ? 0.0 : 1.0;
    auto constOne = rewriter.create<mlir::ConstantOp>(
        op.getLoc(), rewriter.getFloatAttr(resultType, marginalValue));
...
mlir::LogicalResult mlir::spn::HistogramGPULowering::matchAndRewrite(
  ...
  if (op.getSupportMarginal()) {
    ...
    auto ifNaN =
        rewriter.create<scf::IfOp>(op.getLoc(), resultType, isNan, true);
    rewriter.setInsertionPointToStart(&ifNaN.thenRegion().front());
    Value constOne = rewriter.create<mlir::ConstantOp>(
        op.getLoc(), rewriter.getFloatAttr(resultType, marginalValue));
    rewriter.create<scf::YieldOp>(op.getLoc(), constOne);
    rewriter.setInsertionPointToStart(&ifNaN.elseRegion().front());
    Value leaf = processBuckets(buckets, rewriter, index, falseVal, op.getLoc(),
                                computesLog);
...
mlir::Value mlir::spn::HistogramGPULowering::processBuckets(
    llvm::ArrayRef<low::Bucket> buckets, ConversionPatternRewriter &rewriter,
  ...
  if (buckets.size() == 1) {
    ...
    auto lb = rewriter.create<ConstantOp>(
        loc, rewriter.getIntegerAttr(indexVal.getType(),
                                     buckets.front().lb().getInt()));
    auto ub = rewriter.create<ConstantOp>(
        loc, rewriter.getIntegerAttr(indexVal.getType(),
                                     buckets.front().ub().getInt()));
    auto lbCheck =
        rewriter.create<CmpIOp>(loc, CmpIPredicate::sge, indexVal, lb);
    auto ubCheck =
        rewriter.create<CmpIOp>(loc, CmpIPredicate::slt, indexVal, ub);
    ...
    auto probabilityVal = rewriter.create<ConstantOp>(
        loc, rewriter.getFloatAttr(defaultVal.getType(), probability));
    auto selectVal =
        rewriter.create<SelectOp>(loc, rangeCheck, probabilityVal, defaultVal);
    rewriter.restoreInsertionPoint(restore);
    return selectVal;
  } else {
    ...
    auto border = rewriter.create<ConstantOp>(
        loc, rewriter.getIntegerAttr(indexVal.getType(),
                                     right.front().lb().getInt()));
    // Check if the index value is smaller then the first bucket of the right
    // halve.
    auto compare =
        rewriter.create<CmpIOp>(loc, CmpIPredicate::slt, indexVal, border);
    auto ifOp =
        rewriter.create<scf::IfOp>(loc, defaultVal.getType(), compare, true);
    rewriter.setInsertionPointToStart(&ifOp.thenRegion().front());
    auto leftVal =
        processBuckets(left, rewriter, indexVal, defaultVal, loc, computesLog);
    rewriter.create<scf::YieldOp>(loc, leftVal);
    rewriter.setInsertionPointToStart(&ifOp.elseRegion().front());
    auto rightVal =
        processBuckets(right, rewriter, indexVal, defaultVal, loc, computesLog);
...
mlir::LogicalResult mlir::spn::ResolveStripLogGPU::matchAndRewrite(
    mlir::spn::low::SPNStripLog op,
    mlir::spn::low::SPNStripLog::Adaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
  if (op.checkVectorized()) {
    return rewriter.notifyMatchFailure(
        op, "Pattern does not resolve vectorized operation");
  }
  assert(operands.size() == 1);
...
mlir::LogicalResult mlir::spn::ResolveConvertLogGPU::matchAndRewrite(
  ...
  // Simply replace the conversion by its input operand. All users of the
  // conversion should be converted subsequently.
  rewriter.replaceOp(op, operands[0]);
  return success();
}

...
Value BroadcastInsertSuperword::rewrite(Superword *superword,
                                        RewriterBase &rewriter) {
  ...
  Value vectorOp = rewriter.create<vector::BroadcastOp>(
      superword->getLoc(), superword->getVectorType(), broadcastVal);
  for (size_t i = 0; i < superword->numLanes(); ++i) {
    auto element = superword->getElement(i);
    if (element == broadcastVal) {
      continue;
    }
    auto index = conversionManager.getOrCreateConstant(
        superword->getLoc(), rewriter.getI32IntegerAttr((int)i));
    element = stripLogOrValue(element, rewriter);
    vectorOp = rewriter.create<vector::InsertElementOp>(
        superword->getLoc(), element, vectorOp, index);
  }
  return vectorOp;
}

void BroadcastInsertSuperword::accept(PatternVisitor &visitor,
                                      Superword const *superword) const {
  visitor.visit(this, superword);
}
...
ShuffleTwoSuperwords::ShuffleTwoSuperwords(ConversionManager &conversionManager)
    : SLPVectorizationPattern(conversionManager) {
  // The shuffle pattern needs to know which superwords are available for
  // shuffling.
...
LogicalResult ShuffleTwoSuperwords::match(Superword *superword) {
  ...
  // For determinism purposes, use a set vector with deterministic iteration
  // order instead of a set.
  llvm::SmallSetVector<Superword *, 32> relevantSuperwords;
  for (unsigned lane = 0; lane < superword->numLanes(); ++lane) {
    if (superword->hasAlteredSemanticsInLane(lane)) {
      return failure();
    }
    auto const &existingSuperwords =
        superwordsByValue.lookup(superword->getElement(lane));
    if (existingSuperwords.empty()) {
      return failure();
    }
    relevantSuperwords.insert(std::begin(existingSuperwords),
                              std::end(existingSuperwords));
  }
  // Check if any pair of relevant superwords can be combined to convert the
  // target superword.
  for (auto *v1 : relevantSuperwords) {
    SmallPtrSet<Value, 4> remainingValuesFirst(superword->begin(),
                                               superword->end());
    ...
    for (auto *v2 : relevantSuperwords) {
      ...
      for (auto value : *superword) {
        for (size_t index = 0; index < v1->numLanes() + v2->numLanes();
             ++index) {
      ...
      assert(indices.size() == superword->numLanes() &&
             "invalid shuffle match determined");
...
Value ShuffleTwoSuperwords::rewrite(Superword *superword,
                                    RewriterBase &rewriter) {
  assert(shuffleMatches.count(superword) &&
         "no match determined yet for superword");
  ...
  shuffleMatches.erase(superword);
  return rewriter.create<vector::ShuffleOp>(superword->getLoc(), v1, v2,
                                            maskElements);
}

void ShuffleTwoSuperwords::accept(PatternVisitor &visitor,
                                  Superword const *superword) const {
  visitor.visit(this, superword);
}
...
Value VectorizeConstant::rewrite(Superword *superword, RewriterBase &rewriter) {
  ...
  for (auto value : *superword) {
    ...
    } else if (auto op = value.getDefiningOp<SPNConstant>()) {
      // SPNConstant op.
      constants.emplace_back(op.getValueAttr());
    } else {
...
Value CreateConsecutiveLoad::rewrite(Superword *superword,
  ...
  ValueRange indices{batchRead.getDynamicIndex(),
                     conversionManager.getOrCreateConstant(
                         superword->getLoc(),
                         rewriter.getIndexAttr(batchRead.getStaticIndex()))};
  return rewriter.create<vector::LoadOp>(superword->getLoc(),
                                         superword->getVectorType(),
                                         batchRead.getBatchMem(), indices);
}
...
Value CreateGatherLoad::rewrite(Superword *superword, RewriterBase &rewriter) {
  ...
  auto indexType =
      VectorType::get(vectorType.getShape(), rewriter.getI32Type());
  auto indexElements = DenseElementsAttr::get(
      indexType, static_cast<ArrayRef<uint32_t>>(samples));
  ...
  auto maskElements =
      DenseElementsAttr::get(maskType, static_cast<ArrayRef<bool>>(maskBits));
  ...
  auto passThrough =
      conversionManager.getOrCreateConstant(loc, passThroughElements);
...
void VectorizeAdd::accept(PatternVisitor &visitor,
                          Superword const *superword) const {
  visitor.visit(this, superword);
}
...
void VectorizeMul::accept(PatternVisitor &visitor,
                          Superword const *superword) const {
  visitor.visit(this, superword);
}
...
Value VectorizeGaussian::rewrite(Superword *superword, RewriterBase &rewriter) {
  ...
  // Calculate Gaussian distribution using e^((x - mean)^2 / (-2 * variance)) /
  // sqrt(2 * PI * variance)
  ...
  if (vectorType.getElementType().cast<FloatType>().getWidth() == 32) {
    ...
    for (auto value : *superword) {
      meansArray.emplace_back(
          (float)static_cast<SPNGaussianLeaf>(value.getDefiningOp())
              .getMeanAttr()
              .getValueAsDouble());
      float stddev = (float)static_cast<SPNGaussianLeaf>(value.getDefiningOp())
                         .getStddevAttr()
                         .getValueAsDouble();
      variancesArray.emplace_back(-2 * stddev * stddev);
      rootsArray.emplace_back(std::sqrt(2.0f * M_PIf32 * stddev * stddev));
    }
    means = DenseElementsAttr::get(vectorType,
                                   static_cast<ArrayRef<float>>(meansArray));
    variances = DenseElementsAttr::get(
        vectorType, static_cast<ArrayRef<float>>(variancesArray));
    roots = DenseElementsAttr::get(vectorType,
                                   static_cast<ArrayRef<float>>(rootsArray));
  } else {
    ...
    for (auto value : *superword) {
      meansArray.emplace_back(
          static_cast<SPNGaussianLeaf>(value.getDefiningOp())
              .getMeanAttr()
              .getValueAsDouble());
      double stddev = static_cast<SPNGaussianLeaf>(value.getDefiningOp())
                          .getStddevAttr()
                          .getValueAsDouble();
      variancesArray.emplace_back(-2 * stddev * stddev);
    ...
    means = DenseElementsAttr::get(vectorType,
                                   static_cast<ArrayRef<double>>(meansArray));
    variances = DenseElementsAttr::get(
        vectorType, static_cast<ArrayRef<double>>(variancesArray));
    roots = DenseElementsAttr::get(vectorType,
                                   static_cast<ArrayRef<double>>(rootsArray));
  }
  ...
  inputVector =
      util::extendTruncateOrGetVector(inputVector, vectorType, rewriter);
  auto meanVector =
      conversionManager.getOrCreateConstant(superword->getLoc(), means);
  Value gaussianVector = rewriter.create<arith::SubFOp>(
      superword->getLoc(), vectorType, inputVector, meanVector);

  // ((x - mean)^2
  gaussianVector = rewriter.create<arith::MulFOp>(
      superword->getLoc(), vectorType, gaussianVector, gaussianVector);

  // (x - mean)^2 / (-2 * variance)
  auto varianceVector =
      conversionManager.getOrCreateConstant(superword->getLoc(), variances);
  gaussianVector = rewriter.create<arith::DivFOp>(
      superword->getLoc(), vectorType, gaussianVector, varianceVector);

  // e^((x - mean)^2 / (-2 * variance))
  gaussianVector = rewriter.create<math::ExpOp>(superword->getLoc(), vectorType,
                                                gaussianVector);

  // e^((x - mean)^2 / (-2 * variance)) / sqrt(2 * PI * variance)
  auto rootsVector =
      conversionManager.getOrCreateConstant(superword->getLoc(), roots);
  gaussianVector = rewriter.create<arith::DivFOp>(superword->getLoc(),
                                                  gaussianVector, rootsVector);

  if (anyGaussianMarginalized(*superword)) {
    DenseElementsAttr denseOne;
    if (vectorType.getElementType().cast<FloatType>().getWidth() == 32) {
      SmallVector<float, 4> ones(superword->numLanes(), 1.0f);
      denseOne = DenseElementsAttr::get(vectorType,
                                        static_cast<ArrayRef<float>>(ones));
    } else {
      SmallVector<double, 4> ones(superword->numLanes(), 1.0);
      denseOne = DenseElementsAttr::get(vectorType,
                                        static_cast<ArrayRef<double>>(ones));
    }
    auto isNan = rewriter.create<arith::CmpFOp>(superword->getLoc(),
                                                arith::CmpFPredicate::UNO,
                                                inputVector, inputVector);
    auto constOne =
        conversionManager.getOrCreateConstant(superword->getLoc(), denseOne);
    gaussianVector = rewriter.create<arith::SelectOp>(
        superword->getLoc(), isNan, constOne, gaussianVector);
  }

  return gaussianVector;
}

void VectorizeGaussian::accept(PatternVisitor &visitor,
                               Superword const *superword) const {
  visitor.visit(this, superword);
}
...
Value VectorizeLogAdd::rewrite(Superword *superword, RewriterBase &rewriter) {
  // Rewrite 'ln(x + y)' with 'a = ln(x), b = ln(y) and a > b' as 'a + ln(e^(b -
  // a) + 1)'.
  ...
  auto compare = rewriter.create<arith::CmpFOp>(
      superword->getLoc(), arith::CmpFPredicate::OGT, lhs, rhs);
  auto a =
      rewriter.create<arith::SelectOp>(superword->getLoc(), compare, lhs, rhs);
  auto b =
      rewriter.create<arith::SelectOp>(superword->getLoc(), compare, rhs, lhs);
...
void VectorizeLogAdd::accept(PatternVisitor &visitor,
                             Superword const *superword) const {
  visitor.visit(this, superword);
}
...
void VectorizeLogMul::accept(PatternVisitor &visitor,
                             Superword const *superword) const {
  visitor.visit(this, superword);
}
...
Value VectorizeLogGaussian::rewrite(Superword *superword,
                                    RewriterBase &rewriter) {

  auto vectorType = superword->getVectorType();

  // Calculate log-space Gaussian distribution using -ln(stddev) - 0.5 *
  // ln(2*pi) - (x - mean)^2 / (2 * stddev^2)
  DenseElementsAttr means;
  ...
  if (vectorType.getElementType().cast<FloatType>().getWidth() == 32) {
    ...
    means = DenseElementsAttr::get(vectorType,
                                   static_cast<ArrayRef<float>>(meansArray));
    minuends = DenseElementsAttr::get(
        vectorType, static_cast<ArrayRef<float>>(minuendsArray));
    factors = DenseElementsAttr::get(
        vectorType, static_cast<ArrayRef<float>>(factorsArray));
  } else {
    SmallVector<double, 4> meansArray;
    ...
    means = DenseElementsAttr::get(vectorType,
                                   static_cast<ArrayRef<double>>(meansArray));
    minuends = DenseElementsAttr::get(
        vectorType, static_cast<ArrayRef<double>>(minuendsArray));
    factors = DenseElementsAttr::get(
        vectorType, static_cast<ArrayRef<double>>(factorsArray));
  }
  ...
  // (x - mean)^2 / (2 * stddev^2)
  auto factorsVector =
      conversionManager.getOrCreateConstant(superword->getLoc(), factors);
  gaussianVector = rewriter.create<arith::MulFOp>(
      superword->getLoc(), vectorType, gaussianVector, factorsVector);

  // -ln(stddev) - 0.5 * ln(2*pi) - (x - mean)^2 / (2 * stddev^2)
  auto minuendVector =
      conversionManager.getOrCreateConstant(superword->getLoc(), minuends);
  gaussianVector = rewriter.create<arith::SubFOp>(
      superword->getLoc(), vectorType, minuendVector, gaussianVector);

  if (anyGaussianMarginalized(*superword)) {
    DenseElementsAttr denseZero;
    if (vectorType.getElementType().cast<FloatType>().getWidth() == 32) {
      SmallVector<float, 4> zeros(superword->numLanes(), 0.0f);
      denseZero = DenseElementsAttr::get(vectorType,
                                         static_cast<ArrayRef<float>>(zeros));
    } else {
      SmallVector<double, 4> zeros(superword->numLanes(), 0.0);
      denseZero = DenseElementsAttr::get(vectorType,
                                         static_cast<ArrayRef<double>>(zeros));
    }
    auto isNan = rewriter.create<arith::CmpFOp>(superword->getLoc(),
                                                arith::CmpFPredicate::UNO,
                                                inputVector, inputVector);
    auto constZero =
        conversionManager.getOrCreateConstant(superword->getLoc(), denseZero);
    gaussianVector = rewriter.create<arith::SelectOp>(
        superword->getLoc(), isNan, constZero, gaussianVector);
  }

  return gaussianVector;
}

void VectorizeLogGaussian::accept(PatternVisitor &visitor,
                                  Superword const *superword) const {
  visitor.visit(this, superword);
}

  ...
  llvm::sort(std::begin(order), std::end(order),
             [&](Superword *lhs, Superword *rhs) {
               if (depths[lhs] == depths[rhs]) {
                 ...
                 // This maximizes the re-use potential of non-leaf elements in
                 // leaf nodes through extractions.
                 return rhs->isLeaf();
               }
  ...
  for (auto *superword : order) {
    for (size_t lane = 0; lane < superword->numLanes(); ++lane) {
      ...
      if (auto *elementOp = element.getDefiningOp()) {
        ...
        // they aren't computed outside the SLP graph.
        if (!superword->isLeaf()) {
          for (size_t i = 0; i < superword->numOperands(); ++i) {
            ...
            users.erase(
                std::remove(std::begin(users), std::end(users), elementOp),
                std::end(users));
          }
        }
      }
...
void ConversionManager::update(Superword *superword, Value operation,
  ...
  auto scalarInputs =
      leafVisitor.getRequiredScalarValues(appliedPattern, superword);
...
Value ConversionManager::getOrCreateConstant(Location const &loc,
  ...
  auto typedAttribute = attribute.dyn_cast<TypedAttr>();
  assert(typedAttribute && "attribute must be typed");
  assert(
      arith::ConstantOp::isBuildableWith(attribute, typedAttribute.getType()) &&
      "constant op must be buildable");
  auto arithDialect =
...
void reorderOperationsDFS(Block *block) {
  ...
  llvm::SmallSetVector<Operation *, 32> order;

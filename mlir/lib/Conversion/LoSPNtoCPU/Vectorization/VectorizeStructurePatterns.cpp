...
// Print how much of the original function has been covered by all SLP graphs
// combined
...
// Prints how much time each step took (seeding, graph building, pattern
// matching, ...)
...
#define TOP_DOWN_SEEDING true
  ...
  taskBlock->walk([&](Operation *op) { allOps.insert(op); });
#endif

  // In case an SLP graph is not deemed profitable, we don't want to carry
...
VectorizeBatchTask::matchAndRewrite(SPNTask op, SPNTask::Adaptor adaptor,
  ...
  func::FuncOp taskFunc;
  if (failed(createFunctionIfVectorizable(op, adaptor, rewriter, &taskFunc))) {
    return failure();
  }
  ...
  auto taskBlock = taskFunc.addEntryBlock();
  rewriter.setInsertionPointToStart(taskBlock);
  // The number of samples can be derived from the dynamic dimension of one of
  // the input memrefs.

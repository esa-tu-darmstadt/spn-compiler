...
public:
  explicit ReplaceBatchReadWithShuffle(MLIRContext *ctx,
  ...
  LogicalResult matchAndRewrite(low::SPNBatchRead op,
    ...
    // Transposed SPNBatchRead already access memory in vector-like fashion,
    // skipe them during replacement.
    if (op.getTransposed().value_or(false)) {
      return rewriter.notifyMatchFailure(
          op, "Transposed reads are not eligible for replacement");
    }
    //
    // The transformation loads the input values with regular loads (instead of
    // gathers) and transposes a tile of the input using shuffles. The BatchRead
    // does not need to read anything add more, but can just use the
    // transposed/shuffled vector.
    if (!replace.inBounds(op.getStaticIndex()) ||
        !replace[op.getStaticIndex()]) {
      // No replacement found.
      return rewriter.notifyMatchFailure(op, "No replacement defined");
    }
    auto shuffled = replace[op.getStaticIndex()];
    rewriter.replaceOpWithNewOp<low::SPNConvertToScalar>(
        op, shuffled.getType().cast<VectorType>().getElementType(), shuffled);
    return mlir::success();
  }
...
combineHalves(mlir::Value &leftIn, mlir::Value &rightIn, unsigned vectorSize,
              unsigned step, mlir::PatternRewriter &rewriter,
              mlir::Location loc) {
  ...
  auto leftPermutation = rewriter.create<mlir::vector::ShuffleOp>(
      loc, leftIn, rightIn, firstPermutation);
  auto rightPermutation = rewriter.create<mlir::vector::ShuffleOp>(
      loc, leftIn, rightIn, secondPermutation);
  return {leftPermutation, rightPermutation};
}

llvm::SmallVector<mlir::Value, 8>
transposeByPermutation(llvm::ArrayRef<mlir::Value> loadedVectors,
                       unsigned vectorSize, mlir::PatternRewriter &rewriter,
                       mlir::Location loc) {
  ...
  for (unsigned i = 0; i < numPermutationStage; ++i) {
    ...
    for (unsigned j = 0; j < vectorSize / (distance * 2); ++j) {
      ...
      for (unsigned k = 0; k < distance; ++k) {
        ...
        auto outVec =
            combineHalves(leftIn, rightIn, vectorSize, distance, rewriter, loc);
...
struct FuncReplaceGatherWithShuffle : public OpRewritePattern<func::FuncOp> {
  ...
  LogicalResult matchAndRewrite(func::FuncOp func,
                                PatternRewriter &rewriter) const override {
    //
    // Instead of using expensive gather + insert operations to load the same
    // feature for multiple samples into a vector, this transformation will load
    // a vectorWidth x vectorWidth block of input values using ordinary loads.
    // The block will contain vectorWidth features for vectorWidth many samples.
    // To have one feature for multiple samples in one vector, the block is
    // transposed using vector shuffle operations.

    // For each argument of the function which is a MemRef, check if it is
    // eligible for transformation. To be eligible, there must be at least as
    // many vectorized SPNBatchRead using the memref as the vector width.
      ...
      if (arg.getType().isa<MemRefType>()) {
        ...
        for (auto U : arg.getUsers()) {
          if (auto batchRead = dyn_cast<low::SPNBatchRead>(U)) {
            eligible &= true;
            if (batchRead.checkVectorized() &&
                !indices.count(batchRead.getStaticIndex()) &&
                !batchRead.getTransposed().value_or(false)) {
              // Check that we have not encountered the same index before.
              ++useCount;
              minVectorWidth =
                  std::min(minVectorWidth, batchRead.vectorFactor());
    ...
    for (auto inputMem : inputMemories) {
      ...
      // For simplification, we currently only perform this transformation if
      // all vectorized reads are (1) in the same block and (2) have the same
      // vector-width.
      ...
      if (failed) {
        emitWarning(
            loc, Twine("Cannot replace gather with shuffle for input memory "));
        continue;
      }
      ...
      for (unsigned i = 0; i < maxIndex; i += vector_width) {
        ...
        // Check that at least one use of all indices in the current range is
        // present.
        for (unsigned vectorIndex = i; vectorIndex < i + vector_width;
             ++vectorIndex) {
        ...
        changed = true;
        // Load the features [i, i + vector_width) for the samples [batchIndex,
        // batchIndex + vectorWidth)
        ...
        for (unsigned k = 0; k < vector_width; ++k) {
          auto sampleOffset =
              rewriter.create<arith::ConstantOp>(loc, rewriter.getIndexAttr(k));
          auto sampleIndex =
              rewriter.create<arith::AddIOp>(loc, batchIndex, sampleOffset);
          auto inputVector = rewriter.create<vector::TransferReadOp>(
              loc, vectorType, inputMem, ValueRange{sampleIndex, featureIndex});
          inputs.push_back(inputVector);
        }

        auto transposed =
            transposeByPermutation(inputs, vector_width, rewriter, loc);
        for (unsigned k = 0; k < vector_width; ++k) {
          replacements[i + k] = transposed[k];
        }
      }
      //
      // Replace all SPNBatchRead that used the original MemRef with the
      // shuffled vectors.
      RewritePatternSet patterns(func->getContext());
      patterns.insert<ReplaceBatchReadWithShuffle>(func.getContext(),
                                                   replacements);
...
void ReplaceGatherWithShufflePass::runOnOperation() {
  ...
  module->walk([&frozenPatterns](func::FuncOp func) {
    (void)applyOpPatternsAndFold(func.getOperation(), frozenPatterns);
  });
}

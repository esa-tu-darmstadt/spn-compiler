...
ArithmeticPrecisionAnalysis::ArithmeticPrecisionAnalysis(Operation *rootNode) {
  ...
  if (auto module = dyn_cast<ModuleOp>(rootNode)) {
    ...
    llvm::dbgs()
        << "ArithmeticPrecisionAnalysis ctor expected: mlir::ModuleOp\n";
    return;
  }
  ...
  if (queries.size() > 1) {
    llvm::dbgs()
        << "Found more than one query, will only analyze the first query!\n";
  }

  auto query = dyn_cast<spn::high::QueryInterface>(queries.front());

  // FIXME: Static data representation should be replaced by extracting this
  // info from somewhere (e.g. the query?).
  ...
  // The RootNode acts like an interface, the "desired root" can be obtained by
  // calling root() + getDefiningOp().
...
void ArithmeticPrecisionAnalysis::emitDiagnosticInfo() {
  ...
  root->emitRemark("Error Estimation done.").attachNote(root->getLoc())
      << rso.str();
}
...
void ArithmeticPrecisionAnalysis::analyzeGraph(Operation *graphRoot) {
  ...
  format_bits_significance =
      (est_data_representation == data_representation::EM_FLOATING_POINT)
          ? Float_Formats[iterationCount].significanceBits
          : (2 + iterationCount);

  double epsilon =
      std::pow(BASE_TWO, double(-(format_bits_significance + 1.0)));

  switch (est_data_representation) {
  case data_representation::EM_FLOATING_POINT:
    // Make floating-point calculator.
    calc = std::shared_ptr<ErrorEstimationCalculator>(
        new detail::FloatingPointCalculator(epsilon));
    break;
  case data_representation::EM_FIXED_POINT:
    // Make fixed-point calculator.
    calc = std::shared_ptr<ErrorEstimationCalculator>(
        new detail::FixedPointCalculator(epsilon));
    break;
  }
...
void ArithmeticPrecisionAnalysis::estimateLeastMagnitudeBits() {
  ...
  for (auto entry : spn_node_values) {
    ...
    spn_node_value_global_maximum =
        std::max(spn_node_value_global_maximum, value.max);
    spn_node_value_global_minimum =
        std::min(spn_node_value_global_minimum, value.min);
  }

  format_bits_magnitude = calc->calculateMagnitudeBits(
      spn_node_value_global_maximum, spn_node_value_global_minimum);
...
bool ArithmeticPrecisionAnalysis::checkRequirements() {
  ...
  // Floating-point: Check if currently selected format has enough magnitude
  // bits
  if (est_data_representation == data_representation::EM_FLOATING_POINT) {
    // If least amount of magnitude bits is greater than currently selected
    // format's: Search for fitting format.
    if (format_bits_magnitude > Float_Formats[iterationCount].magnitudeBits) {
      ...
      for (int i = iterationCount + 1; i < (int)Float_Formats.size(); ++i) {
        if (format_bits_magnitude > Float_Formats[i].magnitudeBits) {
          // Each time we check a NON-fitting format we can skip the
          // corresponding iteration.
          ++iterationCount;
        } else {
          // A fitting format was found, next iteration will start with a format
          // that provides enough magnitude bits.
          break;
        }
      }
    }
  ...
  case mlir::spn::high::error_model::relative_error:
    err_current = std::abs((value / defect) - 1.0);
    break;
  case mlir::spn::high::error_model::absolute_error:
    err_current = std::abs(value - defect);
    break;
  case mlir::spn::high::error_model::absolute_log_error:
    err_current = std::abs(std::log(std::abs(value / defect)));
    break;
  }

  // Check the error margin; but note that the requirements might already be
  // infeasible.
...
void ArithmeticPrecisionAnalysis::traverseSubgraph(Operation *subgraphRoot) {
  ...
  if (auto leaf = dyn_cast<mlir::spn::high::LeafNodeInterface>(subgraphRoot)) {
    // Handle leaf node.
    if (auto categorical =
            dyn_cast<mlir::spn::high::CategoricalNode>(subgraphRoot)) {
      estimateErrorCategorical(categorical);
    } else if (auto gaussian =
                   dyn_cast<mlir::spn::high::GaussianNode>(subgraphRoot)) {
      estimateErrorGaussian(gaussian);
    } else if (auto histogram =
                   dyn_cast<mlir::spn::high::HistogramNode>(subgraphRoot)) {
    ...
    if (auto sum = dyn_cast<mlir::spn::high::SumNode>(subgraphRoot)) {
      estimateErrorSum(sum);
                   ...
                   dyn_cast<mlir::spn::high::ProductNode>(subgraphRoot)) {
...
ArithmeticPrecisionAnalysis::estimateErrorBinaryOperation(
    SmallVector<ErrorEstimationValue> operands, bool isSum) {
  ...
  if (numOperands == 1) {
    ...
    if (isSum) {
      ...
      // "min": the higher value will be ignored entirely (adder becomes
      // min-selection)
...
void ArithmeticPrecisionAnalysis::estimateErrorSum(
    mlir::spn::high::SumNode op) {
  ...
  for (int i = 0; i < numOperands; ++i) {
    // Assumption: Operands were encountered beforehand -- i.e. their values are
    // known.
    ...
    ErrorEstimationValue w = ErrorEstimationValue{weightValue, weightDefect,
    ...
    weightedAddends.push_back(
        ErrorEstimationValue{accurate, defective, max, min, depth});
  }
  ...
  // Increase depth, since up until now we only took the operands' depth into
  // account.
  v.depth += 1;
  spn_node_values.emplace(op.getOperation(), v);
}

void ArithmeticPrecisionAnalysis::estimateErrorProduct(
    mlir::spn::high::ProductNode op) {
  // Assumption: There are at least two operands.
  auto operands = op->getOperands();
  assert(operands.size() > 1);
...
void ArithmeticPrecisionAnalysis::estimateErrorCategorical(
    mlir::spn::high::CategoricalNode op) {
  double value = 0.0;
  ...
  spn_node_values.emplace(op.getOperation(),
                          ErrorEstimationValue{value, defect, max, min, 1});
}

void ArithmeticPrecisionAnalysis::estimateErrorGaussian(
    mlir::spn::high::GaussianNode op) {
  // Use probability density function (PDF) of the normal distribution
  //   PDF(x) := ( 1 / (stddev * SQRT[2 * Pi] ) ) * EXP( -0.5 * ( [x - mean] /
  //   stddev )^2 )
  double stddev = op.getStddev().convertToDouble();
  // Define c := ( 1 / (stddev * SQRT[2 * Pi] ) )
  double c = std::pow((stddev * std::sqrt(2 * M_PI)), -1.0);

  // Since we do not treat skewed distributions, set value as PDF(mean), which
  // is also the maximum. value := PDF(mean) = c * EXP(0) = c
  double value = c;
  double max = value;
  double defect = calc->calculateDefectiveLeaf(value);

  // Consider 99%, i.e. x := mean +/- (2.576 * stddev)
  // Note 1: mean will be subtracted upon calculating the PDF; hence omit it in
  // the first place: x := 2.576 * stddev Note 2: x will be divided by stddev
  // leaving only constant ~ 2.576. No skew -> PDF is symmetric w.r.t. to
  // PDF(mean), so PDF(mean + delta) = PDF(mean - delta)
...
void ArithmeticPrecisionAnalysis::estimateErrorHistogram(
    mlir::spn::high::HistogramNode op) {
  double value = 0.0;
  double max = std::numeric_limits<double>::min();
  double min = std::numeric_limits<double>::max();
  double defect = 0.0;
...
void ArithmeticPrecisionAnalysis::selectOptimalType() {
  if (!abortAnalysis) {
    ...
    case data_representation::EM_FIXED_POINT:
      selectedType =
          IntegerType::get(root->getContext(),
                           (format_bits_magnitude + format_bits_significance));
      break;
    case data_representation::EM_FLOATING_POINT:
      // Invoke call: Type::get(MLIRContext*)
      selectedType =
          Float_Formats[iterationCount - 1].getType(root->getContext());
      break;
    }
  }
}
...
mlir::Type ArithmeticPrecisionAnalysis::getComputationType(bool useLogSpace) {
  ...
  if (useLogSpace) {
    return mlir::spn::low::LogType::get(
        root->getContext(), mlir::FloatType::getF32(root->getContext()));
  }
  return selectedType;
}

inline double
mlir::spn::detail::FixedPointCalculator::calculateDefectiveLeaf(double value) {
  return (value + EPS);
}

inline double
mlir::spn::detail::FloatingPointCalculator::calculateDefectiveLeaf(
    double value) {
  return (value * ERR_COEFFICIENT);
}

inline double
mlir::spn::detail::FixedPointCalculator::calculateDefectiveProduct(
    ErrorEstimationValue &v1, ErrorEstimationValue &v2) {
  ...
  // Add the corresponding total delta to the already calculated ("accurate")
  // value
  defect +=
      (v1.max * delta_t2) + (v2.max * delta_t1) + (delta_t1 * delta_t2) + EPS;
  return defect;
}

inline double
mlir::spn::detail::FloatingPointCalculator::calculateDefectiveProduct(
    ErrorEstimationValue &v1, ErrorEstimationValue &v2) {
  // Calculate accurate value
  double defect = v1.accurate * v2.accurate;
...
inline double mlir::spn::detail::FixedPointCalculator::calculateDefectiveSum(
    ErrorEstimationValue &v1, ErrorEstimationValue &v2) {
  // Calculate defective value directly
  return (v1.defective + v2.defective);
}

inline double mlir::spn::detail::FloatingPointCalculator::calculateDefectiveSum(
    ErrorEstimationValue &v1, ErrorEstimationValue &v2) {
...
inline int
mlir::spn::detail::FixedPointCalculator::calculateMagnitudeBits(double max,
                                                                double min) {
  return ((int)std::ceil(std::log2(max)));
}

inline int
mlir::spn::detail::FloatingPointCalculator::calculateMagnitudeBits(double max,
                                                                   double min) {
  // Overflow and underflow has to be taken into account.
  // Calculate minimum number of bits (then bias) for 2's complement to
  // determine the number of exponent bits.
  double bias =
      std::ceil(std::abs(std::log2(std::ceil(std::abs(std::log2(min))))));
  bias = std::max(bias, std::ceil(std::abs(
                            std::log2(std::ceil(std::abs(std::log2(max)))))));
  // Calculate actual bias value; to account for special, "unusable" exponent
  // values we do NOT subtract 1.
  bias = std::pow(2.0, bias);
  auto magnitudeBits =
      (int)std::ceil(std::log2(std::ceil(std::abs(std::log2(min))) + bias));
  magnitudeBits = std::max(
      magnitudeBits,
      (int)std::ceil(std::log2(std::ceil(std::abs(std::log2(max))) + bias)));

...
public:
  ...
  LogicalResult matchAndRewrite(SPNTask op,
    ...
    unsigned numNodes = 0;
    {
      ...
      op->walk([&](SPNBody body) {
        ...
        for (auto blockArg : body.getBody().getArguments()) {
          ...
          // All users of the entry block args potentially do not have outside
          // operands.
        ...
        body.getBody().walk([&](Operation *op) {
          if (isa<SPNYield>(op)) {
            ...
            // store the returned results to identify the new task producing the
            // results.
    ...
    for (auto &p : partitions) {
      createTaskForPartition(p, rewriter, op.getLoc(), op.getBatchSize(),
                             inputs, partitions);
    }
...
private:
  ...
  void createTaskForPartition(PartitionInfo partition,
                              PatternRewriter &rewriter, Location loc,
                              unsigned batchSize, InputMap &inputs,
                              llvm::ArrayRef<PartitionInfo> partitions) const {
    // First, collect all input values coming from either outside arguments of
    // the original task or from other partitions.
        ...
        if (!partition.first->contains(operand.getDefiningOp())) {
          if (!inputs.count(operand)) {
            ...
            assert(otherPartition.first &&
                   "Did not find partition producing this value");
    ...
    for (auto *o : partition.first->hasExternalOutputs()) {
      for (auto result : o->getResults()) {
        // Only add to nonPartitionOutputs if there's at least one user outside
        // of the partition.
        auto hasExternalUser =
            llvm::any_of(result.getUsers(), [&partition](auto *U) {
    ...
    for (auto &in : nonPartitionInputs) {
      ...
      bodyInputs.push_back(extract.getResult());
    }
    ...
    for (auto &bodyIn : bodyInputs) {
      if (hasLogType[index++]) {
        bodyBlock->addArgument(
            low::LogType::get(getContext(), bodyIn.getType()), loc);
      } else {
        bodyBlock->addArgument(bodyIn.getType(), loc);
      }
    }
  ...
  /// \return Information about the partition containing the operation/value or
  /// a
  ///         nullptr wrapped in the information if no partition could be found.
  PartitionInfo findPartition(Value input,
                              llvm::ArrayRef<PartitionInfo> partitions) const {
    for (auto &p : partitions) {
      if (p.second) {
        // This partition has already been converted and all outputs should be
        // present in the map.
        continue;
      }
  ...
  void postprocessConstants(llvm::ArrayRef<PartitionInfo> partitions,
        ...
        if (out->hasTrait<OpTrait::ConstantLike>()) {
          ...
          for (auto &use : out->getUses()) {
            if (!p.first->contains(use.getOwner())) {
              ...
              auto otherPart =
                  findContainingPartition(use.getOwner(), partitions);
  ...
  //          nullptr wrapped in the information if no partition could be found.
  PartitionInfo
  findContainingPartition(Operation *op,
                          llvm::ArrayRef<PartitionInfo> partitions) const {
    for (auto &p : partitions) {

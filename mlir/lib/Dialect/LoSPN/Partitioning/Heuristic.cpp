...
void SimpleMoveHeuristic::refinePartitioning() {
  ...
  for (auto *node : nodes) {
    ...
    if (i != maxPartition) {
      // Calculate the positive gain: All edges to users of 'node' that are in
      // partition i+1 will become internal edges.
      int gain = 0;
      bool legal = true;
      for (auto *U : node->getUsers()) {
        ...
        if (partitionID == i + 1) {
          // Although there might be multiple users in the upward partition, the
          // overall gain for all of them combined is only 1, as the result of
          // the node only needs to be stored/loaded to/from memory once for all
          // users in one partition.
          gain = 1;
        }
        if (partitionID == i) {
      ...
      // Calculate the negative gain. All edges from operands of 'node' that are
      // in partition i will become external edges.
    ...
    if (i != 0) {
      // Calculate the positive gain: All edges from operands of 'node' that are
      // in partition i-1 will become internal edges.
      int gain = 0;
      bool legal = true;
      for (auto operand : node->getOperands()) {
        if (externalIn.contains(operand)) {
        ...
        if (!isConstant(defOp) && partitionID == i) {
          // If we have an operand in the same partition, it's illegal to push
          // the node downwards.
          legal = false;
          break;
        }
      }
      // Calculate the negative gain: All edges to users of 'node' that are in
      // partition i will become external edges. As the result of the operation
      // only needs to be stored/loaded to/from memory once for all users, the

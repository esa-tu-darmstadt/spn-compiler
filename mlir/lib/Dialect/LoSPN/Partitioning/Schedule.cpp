...
void Schedule::updateGraph() {
  // Calculate starting times if not done yet
  if (startingTimes_.empty())
    calculateTimes();

  // Update the graph scheduling information
  for (auto &[proc, tasks] : schedule_) {
    for (size_t i = 0; i < tasks.size(); ++i) {
      auto vertex = tasks[i];
      boost::put(SchedulingVertex_StartTime(), graph_, vertex,
                 startingTimes_[proc][i]);
      boost::put(SchedulingVertex_EndTime(), graph_, vertex,
                 endingTimes_[proc][i]);
      boost::put(SchedulingVertex_ProcID(), graph_, vertex, proc);
    }
  }
}

Schedule::Schedule(SchedulingGraph &&graph) : graph_(std::move(graph)) {
  for (auto vertex : boost::make_iterator_range(boost::vertices(graph))) {
    auto cluster = boost::get(SchedulingVertex_ClusterID(), graph, vertex);
    vertexOfTask[cluster] = vertex;
  }
}
...
void Schedule::saveAsHTML(std::string filename,
...
void Schedule::viewSchedule(const TargetExecutionModel &targetModel,
  ...
  saveAsHTML(std::string(Filename.str()), targetModel, title);

  // Display the graph.
  EC = llvm::sys::fs::copy_file(Filename, filename);

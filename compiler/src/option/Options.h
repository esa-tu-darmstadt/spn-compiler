...
public:
  ...
  explicit Opt(std::string key, std::string desc, std::string cat)
      : keyName{std::move(key)}, description(desc), category(cat) {}
  ...
  virtual std::optional<std::unique_ptr<OptValue>>
...
public:
  ...
  static std::vector<llvm::cl::Option *> registerCLOptions();

  /// Set the LLVM command-line options from a map of options.
  /// \param config Configuration.
  static void setCLOptions(const std::map<std::string, std::string> &options);
  ...
  collectCLOptions(const std::vector<llvm::cl::Option *> &clOptions);
};
...
public:
  ...
  explicit Option(std::string k, std::string desc, std::string cat,
                  std::initializer_list<OptModifier *> modifier = {}) noexcept
      : Opt{std::move(k), std::move(desc), std::move(cat)} {
  ...
  /// \param modifier Constraints to attach to this option.
  Option(std::string k, std::string desc, std::string cat, Value defaultVal,
         std::initializer_list<OptModifier *> modifier = {}) noexcept
      : Opt{std::move(k), std::move(desc), std::move(cat)} {
    hasDefault = true;
  ...
  /// parsed value.
  std::optional<std::unique_ptr<OptValue>>
...
public:
  ...
  /// \param modifier Constraints to attach to this option.
  template <typename D>
  EnumOpt(std::string k, std::string desc, std::string cat, D defaultVal,
          std::initializer_list<OptionEnumValue> options,
          std::initializer_list<OptModifier *> modifier = {})
      : Option<int>{k, desc, cat, int(defaultVal), modifier} {
    for (auto &o : options) {
      enumValues.emplace(detail::OptionParsers::toLowerCase(o.name), o);
    }
  }
  ...
  auto &getEnumValues() { return enumValues; }

...
Kernel spn_compiler::compileQuery(const std::string &inputFile,
...
#endif
  } else if (spnc::option::compilationTarget.get(*config) ==
             option::TargetMachine::FPGA) {
  ...
  if (kernel->getKernelType() == KernelType::CLASSICAL_KERNEL) {
    ...
    SPDLOG_INFO("Generated Kernel in {}, kernel name {}", classical.fileName,
                classical.kernelName);
  } else if (kernel->getKernelType() == KernelType::FPGA_KERNEL) {
    FPGAKernel fpga = kernel->getFPGAKernel();
    SPDLOG_INFO("Generated Kernel in {}, project name {}", fpga.fileName,
                fpga.projectName);
  } else {
    assert(false);
  }

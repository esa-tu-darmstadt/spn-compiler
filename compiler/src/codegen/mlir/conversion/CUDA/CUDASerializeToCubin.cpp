...
void mlir::spn::CUDASerializeToCubinPass::getDependentDialects(mlir::DialectRegistry& registry) const {
  registerNVVMDialectTranslation(registry);
  SerializeToBlobPass::getDependentDialects(registry);
}

std::unique_ptr<llvm::Module> mlir::spn::CUDASerializeToCubinPass::translateToLLVMIR(llvm::LLVMContext& llvmContext) {
  ...
  auto llvmModule = mlir::translateModuleToLLVMIR(gpuModule, llvmContext, "LLVMDialectModule");
  if (!llvmModule) {
...
void mlir::spn::CUDASerializeToCubinPass::optimizeNVVMIR(llvm::Module* module) {
  ...
  std::unique_ptr<llvm::TargetMachine> machine(target->createTargetMachine(this->triple,
                                                                           this->chip,
                                                                           this->features, {}, {}));
...
void mlir::spn::CUDASerializeToCubinPass::linkWithLibdevice(llvm::Module* module) {
...
std::unique_ptr<std::vector<char>> mlir::spn::CUDASerializeToCubinPass::serializeISA(const std::string& isa) {

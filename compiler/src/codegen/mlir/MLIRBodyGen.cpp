...
MLIRBodyGen::MLIRBodyGen(mlir::OpBuilder* _builder, std::unordered_map<GraphIRNode*, mlir::Value>* n2v)
...
mlir::Value MLIRBodyGen::getValueForNode(const spnc::NodeReference& node, spnc::arg_t arg) {
  if (!node2value->count(node.get())) {
    node->accept(*this, std::move(arg));
  }
  return node2value->at(node.get());
}
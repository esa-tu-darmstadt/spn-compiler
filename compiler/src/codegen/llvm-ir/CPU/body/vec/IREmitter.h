...
#include "graph-ir/util/GraphIRTools.h"
...
public:
  IREmitter(
      ...
      std::vector<std::vector<NodeReference>>& vectors, InputVarValueMap inputs,
      Function *func, LLVMContext &context, IRBuilder<> &builder,
      Module *module, unsigned width, Value* indVar, const Configuration& config);
...
private:
  ...
  Value *vecArith(WeightedSum &n, Value *acc, Value *in,
    ...
    for (auto &lane : accOrder) {
      auto inputs = ((WeightedSum *)lane.second)->addends();
      double weight = 0.0;
  ...
  template <class T> void emitArith(T &n, arg_t arg) {
	...
	// Sort so that closer values get into the same vector, improving locality
        ...
        std::sort(laneGaussInputs.begin(), laneGaussInputs.end(),
                  [](auto &a, auto &b) {
                    return ((Gauss *)a)->indexVar().index() <
                           ((Gauss *)b)->indexVar().index();
                  });

        std::sort(laneHistoInputs.begin(), laneHistoInputs.end(),
                  [](auto &a, auto &b) {
                    return ((Histogram *)a)->indexVar().index() <
                           ((Histogram *)b)->indexVar().index();
                  });
	...
	Value* histoLoad;
        ...
        if (spnc::option::useGather.get(config)) {
          ...
          for (auto &nodeRef : vecIt.iterators) {
            ...
            Histogram *histObject = (Histogram *)*nodeRef.second;
	  ...
	  bool selectBinary = spnc::option::selectBinary.get(config);
          if (selectBinary) {
            for (auto &nodeRef : vecIt.iterators) {
              Histogram *histObject = (Histogram *)*nodeRef.second;
	...
	if (gaussAccVec && (gaussiansToHandle > 0 || i+1 == laneIdxs.size())) {
	  ...
	  std::unordered_map<size_t, NodeReference> accPositions;
	    ...
	    accPositions[laneNo] = order[laneNo];
            Gauss* nr = (Gauss*) gaussInputs[laneNo][gaussVectorIdx[k]];
	...
	out = gaussAccVec;
      ...
      for (auto& direct : directVec) {
        ...
        for (auto &lane : order) {
          ...
          if (!needed) {
            size_t directInWidth =
                ((VectorType *)directVecVal->getType())->getElementCount().Min;
            // extend the vector width first if the neutral element to be
            // inserted would be outside of the current width
            if (directInWidth <= lane.first) {
	      std::vector<uint32_t> shuffleIndices;
	      for (int i = 0; i <= lane.first; i++) {
		if (i < directInWidth)
		  shuffleIndices.push_back(i);
		else
		  shuffleIndices.push_back(directInWidth);
	      }
              directVecVal = _builder.CreateShuffleVector(
                  directVecVal, UndefValue::get(directVecVal->getType()),
                  shuffleIndices);
            }

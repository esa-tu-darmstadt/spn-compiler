...
IREmitter::IREmitter(
      ...
      Module *module, unsigned width, Value* indVar, const Configuration& _config)
    : _vec(vec), _directVecInputs(directVecInputs), _vectors(vectors), _inputs(inputs),
      _func(func), _context(context), _builder(builder), _module(module),
      simdWidth(width), _indVar(indVar), config(_config) {
...
void IREmitter::visitInputvar(InputVar &n, arg_t arg) {
  ...
  input2value.insert({n.id(), _inputs(n.index(), _indVar)});
}

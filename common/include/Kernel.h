...
// The fields in this struct are alloweded to contain only partially valid
// information. This depends on the use case.
struct FPGAKernel {
  ...
  std::string to_string() const {
    ...
    ss << "FPGAKernel{"
       ...
       << ", kernelId=" << kernelId << ", bodyDelay=" << bodyDelay
       << ", fifoDepth=" << fifoDepth << ", spnVarCount=" << spnVarCount
       ...
       << ", memDataWidth=" << memDataWidth << ", memAddrWidth=" << memAddrWidth
...
public:
  ...
  Kernel(const ClassicalKernel &kernel) : kernel(kernel) {}

  Kernel(const FPGAKernel &kernel) : kernel(kernel) {}

  KernelType getKernelType() const { return KernelType(kernel.index() + 1); }
  ...
  FPGAKernel &getFPGAKernel() { return std::get<FPGAKernel>(kernel); }
};
} // namespace spnc

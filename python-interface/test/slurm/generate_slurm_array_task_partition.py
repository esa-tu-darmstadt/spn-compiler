...
def traverseModels(speakersDir: str, speakersDataDir: str, ratspnDir: str, ratspnDataDir: str, spnSizesFile: str,
                   arrayDir: str, arrayFile: str, kernelDir: str, removeKernels: bool, vectorize: bool, vecLib: str,
                   shuffle: bool, taskPartitions: int, maxAttempts=None, maxSuccessfulIterations=None,
                   maxNodeSize=None, maxLookAhead=None, reorderInstructionsDFS=None, allowDuplicateElements=None,
                   allowTopologicalMixing=None):

    spn_sizes = pandas.read_csv(spnSizesFile, index_col="Name")
    ...
    with open(os.path.join(arrayDir, arrayFile), 'w') as file:
        ...
        for m in models[:-13]:
            ...
            file.write(
                f"{m[0]} {m[1]} {m[2]} {m[3]} {kernelDir} {removeKernels} {vectorize} {vecLib} {shuffle} {optionals_string} {max_task_size}\n"
            )
